...
if __name__ == "__main__":
    ...
    gt0 = np.loadtxt(exp_folder + 'gt.txt', delimiter = ',')
    gt0[:, 0:2] = gt0[:, 0:2] - gt0[0, 0:2]  
    num_users = gt0.shape[0]
    dis_gt0 = calculate_dis_matrix(gt0)
    ...
    for exp_num in [ 1,2,3,4,5]:
        ...
        for f in os.listdir(exp_folder + str(exp_num)):
            if f.endswith(".txt"):
                ...
                # print(dis_matrix)
                if drop:
                    drop_id = np.random.choice([2,3])
                    gt = np.copy(gt0)
                    gt = np.delete(gt, drop_id, 0)
                    dis_gt = np.copy(dis_gt0)
                    dis_gt = np.delete(dis_gt, drop_id, 0)
                    dis_gt = np.delete(dis_gt, drop_id, 1)
                    dis_matrix = np.delete(dis_matrix, drop_id, 0)
                    dis_matrix = np.delete(dis_matrix, drop_id, 1)  
                else:
                    gt = gt0
                    dis_gt = dis_gt0
    ...
    for exp_num in [ 1,2,3,4,5]:
        ...
        for f in os.listdir(exp_folder + str(exp_num)):
            if f.endswith(".txt"):
                ...
                # print(dis_matrix)
                if drop:
                    drop_id = np.random.choice([2,3, 4])
                    gt = np.copy(gt0)
                    gt = np.delete(gt, drop_id, 0)
                    dis_gt = np.copy(dis_gt0)
                    dis_gt = np.delete(dis_gt, drop_id, 0)
                ...
                if np.amax(tmp_2d) > 8 or np.amax(tmp_AOA) > 25:
                    print("Warning big error!!!!: ", np.amax(tmp_2d), np.amax(tmp_AOA))
                    print(dis_matrix2)
                    print(dis_matrix2 - dis_gt) # # print(AOA_err)
...
print(len(err_2d))
plot_cdf([np.array(err_2d2),np.array(err_2d)], ['2d err with 5 node', '2d err with 4 node'])
plot_cdf([np.array(err_2d)], ['2d err'])
# plot_cdf([np.array(err_dis[0]), np.array(err_dis[1]), np.array(err_dis[2])], ['0-10m', '10-15', '15-25'])

np.save("result/4node_5node/err_5node.npy", np.array(err_2d2))
np.save("result/4node_5node/err_4node.npy", np.array(err_2d))
