...
%     reply_interval = 0.8 *fs;
    ...
    for idx = 2:length(locs)
        ...
        %%%% leader's chirp
%         begin_indexes(1) = self_idx - reply_interval - interval*user_id - variance_sample;
% %         leader_chirp =  rx_signal(self_idx +1 : self_idx + sig_len);
%         
%         for chirp_id = 0:(group_size-2)
%             begin_indexes(chirp_id + 2) = (chirp_id-user_id)*interval + self_idx - variance_sample;
%         end
